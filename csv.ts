...
const entryPoints = ['api', 'css', 'javascript'];
...
const getReleaseDateMap = (
  browsers: BrowserName[],
): Map<BrowserName, Map<string, string>> => {
  const dateMap = new Map<BrowserName, Map<string, string>>();
...
const main = () => {
  ...
  console.log(
    `path,deprecated,experimental,standard_track,count,first_date,last_date,${columns.join(
      ',',
    )},comments`,
  );
  for (const { path, compat } of walk(entryPoints, bcd)) {
    ...
    const support = browsers.flatMap((browser) => {
      ...
      if (firstRange.impl_url) {
        links.push(firstRange.impl_url);
      }
    ...
    if (bug) {
      crbug = `=HYPERLINK(${JSON.stringify(bug)};${JSON.stringify(
        bug.substr(8),
      )})`;
    } else {
      bug = '';
